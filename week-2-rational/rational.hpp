...
public:
   ...
   /// decimal, hexadecimal, oct, and setw(4)'s.
   friend std::ostream & operator<<( std::ostream & lhs, const rational & rhs ){
      return lhs 
         << "[" << rhs.counter << "/"  << rhs.denominator << "}" << " "
         << std::showbase << "[" << std::hex << rhs.counter << "/" << std::hex << rhs.denominator << "}" << " "
         << std::showbase << "[" << std::oct << rhs.counter << "/" << std::oct << rhs.denominator << "}" << " "
         << "[" << std::setw(4)<< rhs.counter << "/" << std::setw(4) << rhs.denominator << "}";
   ...
   /// \brief
   /// Multiply the rational, and return itself.
   /// \details
   /// This operator*= mutliplies the given rational with anoter rational, than returns itself (rational).
   rational operator*=( const rational & rhs ){
